<template>
  <div class="app-container">
    <el-alert
      v-if="pendingCount > 0"
      :title="'您有 ' + pendingCount + ' 条待处理报修，请及时处理'"
      type="warning"
      show-icon
      :closable="false"
      class="mb8"
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-bell" size="mini" @click="filterPending">查看待处理</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="requestList">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="180">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-view" @click="handleView(scope.row)" v-hasPermi="['repair:worker:query']">查看</el-button>
          <el-button v-if="scope.row.status !== '3' && scope.row.status !== '2'" size="mini" type="text" icon="el-icon-edit" @click="handleProcess(scope.row)" v-hasPermi="['repair:worker:edit']">处理</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import { listRepairRequest, getRepairRequest, handleRepairRequest, getPendingRepairCount } from '@/api/repair/request'
...
export default {
  ...
  created() {
    this.getList()
    this.refreshPendingCount(true)
    this.pollTimer = setInterval(() => this.refreshPendingCount(false), 60000)
  },
  beforeDestroy() {
    if (this.pollTimer) {
  ...
  methods: {
    ...
    refreshPendingCount(silent) {
      getPendingRepairCount().then(response => {
        const count = response.data || 0
        if (!silent && count > this.lastPendingCount) {
          this.$notify({
            title: '新的待处理报修',
            message: '有 ' + (count - this.lastPendingCount) + ' 条新的待处理报修',
            type: 'warning',
            duration: 5000
          })
          this.getList()
